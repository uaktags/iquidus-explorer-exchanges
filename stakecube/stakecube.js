...
var base_url = 'https://stakecube.net/app/api/v1/exchange';

function get_summary(coin, exchange, cb) {
  var summary = {};
  var url=base_url + '/tickers?target=' + coin.toUpperCase() + '&base=' + exchange.toUpperCase();
  request({uri: url, json: true}, function (error, response, body) {
    if (error) {
      ...
      summary['volume'] = parseFloat(body[0]['volume']).toFixed(8);
      ...
      summary['last'] = parseFloat(body[0]['last_price']).toFixed(8);
...
function get_trades(coin, exchange, cb) {
  var req_url = base_url + '/trades?target=' + coin.toUpperCase() + '&base=' + exchange.toUpperCase();
...
function get_orders(coin, exchange, cb) {
  var req_url = base_url + '/orders?target=' + coin.toUpperCase() + '&base=' + exchange.toUpperCase();
    request({ uri: req_url, json: true }, function (error, response, body) {
        if(error)
            return cb(error, null);
        else {//if (body.error !== true) {
